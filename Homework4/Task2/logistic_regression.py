...
results.append(evaluate(lr_model, Xr_test, yr_test, "Train RED -> Test RED (in-domain)"))
results.append(evaluate(lr_model, Xw_test, yw_test, "Train RED -> Test WHITE (cross-domain)"))

# Train on WHITE
lr_model.fit(Xw_train, yw_train)
results.append(evaluate(lr_model, Xw_test, yw_test, "Train WHITE -> Test WHITE (in-domain)"))
results.append(evaluate(lr_model, Xr_test, yr_test, "Train WHITE -> Test RED (cross-domain)"))

# Save metrics table for Part C
metrics_df = pd.DataFrame(results)
metrics_df.to_csv("artifacts/logreg_metrics.csv", index=False)
print("\nSaved metrics to artifacts/logreg_metrics.csv")
print("\n", metrics_df)
