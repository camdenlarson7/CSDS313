...
def scatter_with_fit(x, y, cols, outpath):
    ...
    plt.title(f"p2b: {cols[0]} vs {cols[1]}")
    plt.tight_layout()
...
def main():
    ap = argparse.ArgumentParser(description="Task 2(b): Pearson correlation for p2b.csv")
    ap.add_argument("--data", default=os.path.join("..", "data", "p2b.csv"),
                    help="Path to p2b.csv (default: ../data/p2b.csv from src/)")
    ap.add_argument("--cols", nargs=2, default=None,
                    help="Two column names (default: use the only two in the file).")
    ap.add_argument("--alpha", type=float, default=0.05,
                    help="Significance level alpha (default: 0.05).")
    ap.add_argument("--plot", action="store_true",
                    help="If set, save a scatter plot with best-fit line to ../results/p2b_scatter.png")
    ap.add_argument("--header", choices=["infer","none"], default="infer",
                    help="CSV header handling: 'infer' (default) or 'none' (force no header).")
    args = ap.parse_args()
    ...
    print("="*70)
    print("Task 2(b): Pearson correlation for two variables")
    ...
    if args.plot:
        outpath = os.path.join("..", "results", "p2b_scatter.png")
